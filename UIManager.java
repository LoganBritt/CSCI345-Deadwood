...
public class UIManager {
		...
		while (playerNumInput < 2 || playerNumInput > 8) {
			...
			if (playerNumInput < 2 || playerNumInput > 8) {
				if (numInputFail != 2 && numInputFail != 5) {
					System.out
							.println("I'm sorry, that's outside the boundaries (2-8 Players Only). Please try again:");
					// numInputFail++;
		...
		while (GameManager.getDay() != 0) {
			...
			switch (input) {
				...
				case "stats all":
					...
					break;
				case "space":
					printSpace(actvPlayer);
					break;
				case "space all":
					printSpaceAll();
					break;
				case "card":
					printCard(actvPlayer);
					break;
				case "role":
					printPlayerRole(actvPlayer);
					break;
				case "act":
					printAct(actvPlayer);
					break;
	...
	private static void printHelp() {
		...
		System.out.println(
				"* Rehearse (Ex 'Rehearse'): Rehearse and get a rehearsal token. You can only rehearse if you're working on a role");
	...
	// Prints all of the Spaces and their information
	public static void printSpaceAll() {
		BoardManager.printAllSpaces();
	}

	// Prints everything the player will need to know about the card of the scene
	// they're one
	...
	// Prints all the info the player needs to know about the role they're working
	// on
	...
	private static void printUpgrade(Player actvPlayer, String input) {
		...
		if (!(paymentType.toLowerCase().equals("c") || paymentType.toLowerCase().equals("d")
				|| paymentType.toLowerCase().startsWith("dollars")
				|| paymentType.toLowerCase().startsWith("credits"))) {
	...
	private static void printRoleChange(Player player, String input) {
		...
		for (int i = 0; i < untakenRoles.length; i++) {
			if (untakenRoles[i] != null && !(untakenRoles[i].isTaken())
					&& untakenRoles[i].getTitle().toLowerCase().equals(newRole.toLowerCase())) {
		...
		if (foundRole == null) {
			System.out
					.println("Either the role you entered is not on this scene or is already taken by another player.");
